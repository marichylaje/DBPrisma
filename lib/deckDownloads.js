function parseDeckKey(deckKey) {
  ...
  if (value.startsWith('precon:')) {
    ...
    if (!name) return null;
    return { type: 'precon', id: null, deckKey: name };
  }

  if (value.startsWith('influencer:')) {
    const name = value.slice('influencer:'.length).trim();
    if (!name) return null;
    return { type: 'influencer', id: null, deckKey: name };
  }
...
function buildDownloadCountsResponse(deckKeys, countsByKey) {
  ...
  return list.reduce((acc, key) => {
    ...
    const resolvedKey = parsed.type === 'cloud' ? `cloud:${parsed.id}` : `precon:${parsed.deckKey}`;
